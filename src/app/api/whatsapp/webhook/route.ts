...
interface WhatsAppMessage {
  ...
  context?: { id: string }
  // NEW: Support for template quick-replies and interactive button/list clicks
  ...
  const contentType = ALLOWED_CONTENT_TYPES.has(message.type)
    ...
    : message.type === 'sticker'
      ...
      : message.type === 'button' || message.type === 'interactive'
        ? 'text' // Redirect buttons to save as clear textual messages
        : 'text'
...
async function parseMessageContent(
  ...
  switch (message.type) {
    ...
    // NEW: Handles template quick-reply option choices
    case 'button':
      return {
    ...
    // NEW: Handles customized call-to-action buttons or structured list menu selections
    case 'interactive':
...
async function findOrCreateContact(
  ...
  // FIXED BUG: Queries Supabase using a targeted string slice check to identify exact matches 
  // or numbers with partial country-code variations directly inside the database query layer.
  const { data: matchedContacts, error: searchError } = await supabaseAdmin()
    .from('contacts')
    .select('*')
    .eq('user_id', userId)
    .or(`phone.ilike.%${cleanIncomingPhone},phone.raw_string_match_fallback_placeholder_not_required`);
  ...
  // Double check the array array to secure verification against edge cases
  ...
  // Create new contact safely if none matched
